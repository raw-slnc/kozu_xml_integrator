...
## Known limitations of cross-referencing / 森林計画図との照合における既知の制約

森林計画図・森林簿と公図（法務局地図XML）の市町村・大字を突き合わせる際、表記の違いで一致しないことがあります。

- **合併前の旧市町村名**：合併で他市に編入された区域が、森林計画図・森林簿側で旧市町村名のまま表記されることがあります。本プラグインは先頭の「旧」や「、／※／（…）」の飾りを外して照合しますが、旧町村名が現行市名と別語だと一致しません。例：静岡市に編入された「旧井川村」は「井川村」となり「静岡市」と一致しない。
- **政令指定都市の区名**：森林計画図側の大字に区名が付くことがあります（公図側は区名なし）。完全一致が外れたときは先頭の区名（1〜4文字＋「区」）を外して再試行しますが、これはヒューリスティックです。例：「葵区井川」⇔ 公図「井川」。
- **同名大字**：異なる市町村に同じ大字名があり、その一部だけを取り込んでいる場合、地番検索が別市町村の筆に当たることがあります。対象市町村のXMLを取り込んでください。
